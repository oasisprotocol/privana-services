    ...
    async def _run_internal_once(self) -> None:
        ...
        async with lp_transfer_lock:
            if active:
                ...
                return
            # Also protects against a previous broadcast whose response/hash
            # was lost: don't sign against ledger state while EVM txs are pending.
            pending, confirmed = await asyncio.to_thread(
                lambda: (
                    sapphire.w3.eth.get_transaction_count(sapphire.account.address, "pending"),
                    sapphire.w3.eth.get_transaction_count(sapphire.account.address, "latest"),
                )
            )
            ...
            for index, swap in enumerate(prepared):
                try:
                    ...
                    tx_hash = await asyncio.to_thread(
                        sapphire.submit_contract_call, **self._call(args), gas_limit=1_000_000
                    )
