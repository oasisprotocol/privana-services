...
# EarnManager offsets conversions by VIRTUAL_SHARES/VIRTUAL_ASSETS, so pool
# totals are chosen to make (assets + 1) / (shares + 1_000_000) an exact rate.
TOTAL_SHARES = 999_000_000
RATE_1_00 = 999_999_999
RATE_1_05 = 1_049_999_999
RATE_0_98 = 979_999_999
RATE_0_90 = 899_999_999
...
class TestChange24h:
    def test_positive_yield(self, test_db):
        _store_rate(ANCHOR, RATE_1_00, TOTAL_SHARES)  # rate 1.0
        result = change_24h(USER, POOL, 500, RATE_1_05, TOTAL_SHARES, NOW)  # rate 1.05
        assert result is not None
        assert result.amount == "25"  # 500*1.05 - 500*1.0
        assert result.pct == "0.050000"

    def test_flat_rate_is_zero_not_null(self, test_db):
        _store_rate(ANCHOR, RATE_1_00, TOTAL_SHARES)
        result = change_24h(USER, POOL, 500, RATE_1_00, TOTAL_SHARES, NOW)
        assert result is not None
    ...
    def test_loss_is_negative_not_clamped(self, test_db):
        _store_rate(ANCHOR, RATE_1_00, TOTAL_SHARES)
        result = change_24h(USER, POOL, 500, RATE_0_98, TOTAL_SHARES, NOW)
        assert result is not None
        assert result.amount == "-10"
        assert result.pct == "-0.020000"

    def test_uses_newest_sample_at_or_before_window_start(self, test_db):
        _store_rate(ANCHOR - 21600, RATE_0_90, TOTAL_SHARES)
        _store_rate(ANCHOR, RATE_1_00, TOTAL_SHARES)
        result = change_24h(USER, POOL, 500, RATE_1_05, TOTAL_SHARES, NOW)
        assert result.amount == "25"

    def test_no_identity_returns_none(self, test_db):
        _store_rate(ANCHOR, RATE_1_00, TOTAL_SHARES)
        assert change_24h(None, POOL, 500, RATE_1_05, TOTAL_SHARES, NOW) is None
        assert change_24h("", POOL, 500, RATE_1_05, TOTAL_SHARES, NOW) is None

    def test_zero_shares_returns_none(self, test_db):
        _store_rate(ANCHOR, RATE_1_00, TOTAL_SHARES)
        assert change_24h(USER, POOL, 0, RATE_1_05, TOTAL_SHARES, NOW) is None

    def test_no_sample_old_enough_returns_none(self, test_db):
        _store_rate(NOW - WINDOW_SEC + 3600, RATE_1_00, TOTAL_SHARES)  # only 23h old
        assert change_24h(USER, POOL, 500, RATE_1_05, TOTAL_SHARES, NOW) is None
    ...
    def test_sample_exactly_at_the_boundary_is_accepted(self, test_db):
        ...
        _store_rate(NOW - WINDOW_SEC, RATE_1_00, TOTAL_SHARES)

        assert change_24h(USER, POOL, 500, RATE_1_05, TOTAL_SHARES, NOW) is not None
    ...
    def test_legacy_row_without_observed_at_is_judged_conservatively(self, test_db):
        ...
        db_write(
            ...
            (POOL, NOW - WINDOW_SEC, str(RATE_1_00), str(TOTAL_SHARES)),
        )

        assert change_24h(USER, POOL, 500, RATE_1_05, TOTAL_SHARES, NOW) is None

    def test_stale_sample_returns_none(self, test_db):
        _store_rate(NOW - MAX_SAMPLE_AGE_SEC - 21600, RATE_1_00, TOTAL_SHARES)
        assert change_24h(USER, POOL, 500, RATE_1_05, TOTAL_SHARES, NOW) is None
    ...
    def test_user_match_is_case_insensitive(self, test_db):
        _store_rate(ANCHOR, RATE_1_00, TOTAL_SHARES)
        _insert_cashflow(created_at=NOW - 3600)
        assert change_24h(USER.upper().replace("0X", "0x"), POOL, 500, RATE_1_05, TOTAL_SHARES, NOW) is None

    def test_zero_base_returns_none(self, test_db):
        _store_rate(ANCHOR, 0, TOTAL_SHARES)
        assert change_24h(USER, POOL, 500, RATE_1_05, TOTAL_SHARES, NOW) is None

    def test_zero_total_shares_in_sample_returns_none(self, test_db):
        _store_rate(ANCHOR, RATE_1_00, 0)
        assert change_24h(USER, POOL, 500, RATE_1_05, TOTAL_SHARES, NOW) is None

    def test_identity_matches_value_difference(self, test_db):
        # change == value_now - value_then, valued the way the contract values
        # a position on both ends of the window.
        virtual_shares, virtual_assets = 1_000_000, 1
        _store_rate(ANCHOR, RATE_1_00, TOTAL_SHARES)
        shares = 4321
        result = change_24h(USER, POOL, shares, RATE_1_05, TOTAL_SHARES, NOW)
        denominator = TOTAL_SHARES + virtual_shares
        expected = (
            shares * (RATE_1_05 + virtual_assets) // denominator
            - shares * (RATE_1_00 + virtual_assets) // denominator
        )
