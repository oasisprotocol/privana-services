...
def _stub_executor(settings):
    import src.services.swap.scheduler as sched_mod
    ...
    sched_mod._scheduler_instance = executor
    return executor


def _stub_lifi_worker():
    import src.services.swap.lifi as lp_mod
    from src.services.swap.worker import SwapWorker

    worker = SwapWorker(
        name="lifi",
        claim_sql=lp_mod.LIFI_CLAIM_SQL,
        claim_params=lp_mod.LIFI_CLAIM_PARAMS,
        execute=lp_mod._pipeline_instance.execute_swap,
    )
    return worker
