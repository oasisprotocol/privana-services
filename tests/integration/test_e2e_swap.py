...
async def api_client():
    ...
    ip_mod._internal_worker = None
    lp_mod._pipeline_instance = None
    lp_mod._lifi_worker = None

    from src.main import app
    from src.services.swap.internal import get_internal_worker
    from src.services.swap.lifi import get_lifi_worker
    transport = httpx.ASGITransport(app=app)
    get_internal_worker().start()
    get_lifi_worker().start()
    try:
        async with httpx.AsyncClient(transport=transport, timeout=120, base_url="http://test") as c:
            yield c
    finally:
        await get_internal_worker().stop()
        await get_lifi_worker().stop()
