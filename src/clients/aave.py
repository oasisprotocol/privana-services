        ...
        for attempt in range(1, ESTIMATE_REVERT_ATTEMPTS + 1):
            try:
                ...
                if attempt == ESTIMATE_REVERT_ATTEMPTS:
                    raise RuntimeError(
                        f"Aave {function_name} would revert: {exc}"
                    ) from exc
                logger.warning(
                    "Aave %s estimation reverted (attempt %d/%d), retrying: %s",
                    function_name, attempt, ESTIMATE_REVERT_ATTEMPTS, exc,
                )
                time.sleep(ESTIMATE_REVERT_BACKOFF_SEC)
            except Exception as exc:
                logger.warning(
                    "Aave %s gas estimation failed (%s); using default %d",
        ...
        for attempt in range(1, RECEIPT_RETRY_ATTEMPTS + 1):
            try:
                ...
                if attempt < RECEIPT_RETRY_ATTEMPTS:
                    logger.warning(
                        "Aave receipt read failed (attempt %d/%d), retrying: %s",
